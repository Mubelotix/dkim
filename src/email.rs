...
fn test_signing_simple() {
    ...
    let mail = mail.sign(DkimHeader::new("mubelotix.dev".to_string(), "common".to_string()).with_canonicalization((CanonicalizationType::Simple, CanonicalizationType::Simple)), &key).unwrap();
    let mail = Email::try_from(mail.as_str()).unwrap();
