use nom::{IResult, bytes::complete::{take_while1, tag}};
use crate::parsing::{tag_value_list::*, ParsingError, quoted_printable::from_dqp};
...
pub fn dns_record_tag<'a>(name: &'a str, input: &'a str) -> IResult<&'a str, Tag<'a>, ParsingError> {
    Ok(match name {
        "v" => {
...
mod test {
    ...
    fn test_txt_record_parsing() {
        ...
        assert_eq!(tag_list("v=DKIM1; h=sha1:sha256; n=this=20is=20a=20test; s=email; t=y:s:z; other=mubelotix", &dns_record_tag).unwrap(), vec![
            Tag::Version("DKIM1"),
    ...
    fn test_colon_separated_value() {
        assert_eq!(colon_separated_values("first:second:third").unwrap().1, vec![
            "first",
            "second",
            "third"
        ]);
        ...
        assert_eq!(colon_separated_values("first: \r\n  second:third").unwrap().1, vec![
            "first",
            "second",
            "third"
        ]);

        assert_eq!(colon_separated_values("first").unwrap().1, vec![
            "first"
        ]);

        assert_eq!(colon_separated_values("first:second:third\r\n :fourth").unwrap().1, vec![
            "first",
            "second",
            "third",
            "fourth"
