...
pub fn get_signature<'a, 'b>(
    raw_mail: &'a str,
    header: Signature<'b>,
    private_key: &'a rsa::RSAPrivateKey,
) -> Result<Signature<'b>, SigningError> {
...
pub fn get_signature_parsed<'a, 'b>(
    parsed_mail: MAIL<'a>,
    mut header: Signature<'b>,
    private_key: &'a rsa::RSAPrivateKey,
) -> Result<Signature<'b>, SigningError> {
    // canonicalization
    let owned_body;
    let mut body = match header.canonicalization.0 {
        CanonicalizationType::Relaxed => {
            owned_body = canonicalize_body_relaxed(parsed_mail.1.to_string());
            &owned_body
        }
        CanonicalizationType::Simple => canonicalize_body_simple(parsed_mail.1),
    };
...
mod test {
    ...
    fn test_signing_sha1() {
        ...
        let signature = get_signature(&mail, Signature::new("mubelotix.dev", "common").with_algorithm(SigningAlgorithm::RsaSha1), &private_key).unwrap();
        mail.insert_str(mail.find("\r\n\r\n").unwrap(), &format!("\r\n{}", signature.to_string()));
    ...
    fn test_signing_simple() {
        ...
        let signature = get_signature(&mail, Signature::new("mubelotix.dev", "common").with_canonicalization((CanonicalizationType::Simple, CanonicalizationType::Simple)), &private_key).unwrap();
        mail.insert_str(mail.find("\r\n\r\n").unwrap(), &format!("\r\n{}", signature.to_string()));
